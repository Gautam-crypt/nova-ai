...
# ═══════════════════════════════════════════════════════
# PHASE 6: Web Dashboard (NOVA Command Center) & API Fixes
# ═══════════════════════════════════════════════════════

The user requested a "badass" (taga) web dashboard so they don't have to look at the terminal anymore. It will have a premium UI using React (Vite) and Vanilla CSS (no Tailwind as per core instructions, custom styling with glassmorphism).

## 1. API Fixes (jarvis/api/server.py & nova-mobile)
Before building the dashboard, we must fix the API routes so both the Mobile App and the Web Dashboard can communicate with the NOVA backend:
- Add `GET /findings` to `jarvis/api/server.py` to fetch security alerts.
- Update `nova-mobile/lib/api-client.ts` to point to `/chat` instead of `/api/command` and fix the paths for `/status` and `/findings`.

## 2. Frontend Project Setup
- Create a new React Vite project in the `frontend` folder.
- Run `npm install axios` for API communication.
- Create a rich, dark-mode CSS design system (`index.css`) with glassmorphism and modern micro-animations.

## 3. Dashboard Features
The dashboard will have a sidebar and a main content area.
- **Live Chat / Console**: Talk to the ReAct engine directly, view its thoughts and outputs.
- **KAVACH Security**: A dedicated view showing the live `findings_queue` with HIGH/LOW priority alerts.
- **System Metrics**: Live CPU, RAM, and Chronos tracking stats.

### Proposed File Structure for `frontend/`
- `index.css`: Global styles, CSS variables for dark theme, animations.
- `src/App.jsx`: Main Layout and routing/state.
- `src/components/ChatBox.jsx`: The chat interface with NOVA.
- `src/components/SecurityAlerts.jsx`: KAVACH dashboard.
- `src/components/SystemStatus.jsx`: Live metrics.

## User Review Required
> [!IMPORTANT]
> The Web Dashboard will run locally on `http://localhost:5173`. Do you want me to automatically serve this dashboard through the existing Python `main.py` server (on port 8080) so you only have to run one command, or keep it separate so you can run `npm run dev` in the `frontend` folder for faster UI updates?
